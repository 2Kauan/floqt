...
import { X, Folder as FolderIcon, Check, Quote } from 'lucide-react';
import { Button } from '../common/Button';
import { Highlight } from '../../types';
import { useHighlightFolders } from '../../hooks/useFolders';
import { moveHighlightToFolder } from '../../services/folderService';
import { useToastStore } from '../../store/useToastStore';

export interface MoveHighlightModalProps {
  isOpen: boolean;
...
export function MoveHighlightModal({
  isOpen,
  highlight,
  bookId,
  onClose,
  onSuccess,
}: MoveHighlightModalProps) {
  const { folders, getFolderTree } = useHighlightFolders(bookId);
  ...
  useEffect(() => {
    if (isOpen && highlight) {
      setSelectedFolderId(highlight.folderId || null);
    }
  }, [isOpen, highlight]);
  ...
  const handleSave = async () => {
    try {
      ...
      await moveHighlightToFolder(highlight.id, selectedFolderId);
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="move-highlight-modal-title"
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-border">
          <div className="flex items-center gap-2">
            <FolderIcon className="w-5 h-5 text-accent" />
            <h2 id="move-highlight-modal-title" className="text-lg font-serif font-bold text-ink">
              Mover Destaque para Pasta
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <p className="text-xs text-ink-muted mt-3 mb-4 line-clamp-2 italic">
        ...
        <div className="max-h-60 overflow-y-auto space-y-1.5 p-1 border border-border/70 rounded-xl bg-bg/50">
          ...
          >
            <div className="flex items-center gap-2">
              <Quote className="w-4 h-4 text-ink-muted" />
              <span>Raiz do Livro (Sem pasta)</span>
            </div>
